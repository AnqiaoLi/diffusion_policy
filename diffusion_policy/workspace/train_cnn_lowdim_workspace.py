...
from diffusion_policy.policy.cnn_lowdim_policy import CNNLowdimPolicy
...
# %%
class TrainCNNLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: CNNLowdimPolicy
        self.model = hydra.utils.instantiate(cfg.policy)

        self.ema_model: CNNLowdimPolicy = None
    ...
    def run(self):
        ...
        if cfg.training.resume:
            self._output_dir = cfg.training.resume_checkpoint_dir
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if debug:
                            ...
                            mse = torch.nn.functional.mse_loss(action, gt_action)
                            ema_result = self.model.predict_action(obs_dict)
                            ema_action = ema_result['action']
                            ema_mse = torch.nn.functional.mse_loss(ema_action, gt_action)
                            print(f"mse: {mse.item()}, ema_mse: {ema_mse.item()}")
...
def main(cfg):
    workspace = TrainCNNLowdimWorkspace(cfg)
    workspace.run()
